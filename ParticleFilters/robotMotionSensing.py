...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if(beta > 0.001):
          turning_radius = distance/beta
          cx = self.x - sin(theta)*turning_radius
          cy = self.y + cos(theta)*turning_radius

          x = cx + sin(theta + beta)*turning_radius
          y = cy - cos(theta + beta)*turning_radius
          theta = (theta + beta)%(2*pi)
        else:
          x = self.x + distance*cos(theta)
          y = self.y + distance*sin(theta)
          theta = (theta + beta)%(2*pi)
    ...
    def sense(self, add_noise = 1): #do not change the name of this function
        Z = []
        for i in landmarks:
            bearing = atan2(i[0] - self.y,i[1] -  self.x) - self.orientation
            if add_noise:
